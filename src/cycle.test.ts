...
test('cycleFor rolls back just before a boundary', () => {
  ...
  assert.equal(c.id, '2026-06-11T00:00:00.000Z');
  assert.equal(c.windowStart, '2026-06-11T00:00:00.000Z');
});

test('cycleFor produces the four canonical UTC boundaries', () => {
  assert.equal(cycleFor(new Date('2026-06-11T00:00:00Z')).id, '2026-06-11T00:00:00.000Z');
  assert.equal(cycleFor(new Date('2026-06-11T06:00:00Z')).id, '2026-06-11T06:00:00.000Z');
  assert.equal(cycleFor(new Date('2026-06-11T12:00:00Z')).id, '2026-06-11T12:00:00.000Z');
  assert.equal(cycleFor(new Date('2026-06-11T23:59:00Z')).id, '2026-06-11T18:00:00.000Z');
});
...
test('previousCycle / nextCycle shift the window by exactly 6h', () => {
  ...
  assert.equal(previousCycle(c).id, '2026-06-11T00:00:00.000Z');
  assert.equal(nextCycle(c).id, '2026-06-11T12:00:00.000Z');
  // round-trip
...
test('previousCycle crosses a day boundary correctly', () => {
  ...
  assert.equal(previousCycle(c).id, '2026-06-10T18:00:00.000Z');
});
