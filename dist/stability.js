...
export function indexByCluster(previous) {
    const map = new Map();
    if (!previous)
        return map;
...
export function incumbentIds(previous) {
    const ids = new Set();
    if (!previous)
        return ids;
    for (const entry of previous)
 ...
 */
export function computeDelta(entry, previous) {
    const prev = indexByCluster(previous).get(entry.clusterId);
    if (!prev)
        return { previousRank: null, movement: 'new' };
    if (entry.rank < prev.rank)
        return { previousRank: prev.rank, movement: 'up' };
    if (entry.rank > prev.rank)
        return { previousRank: prev.rank, movement: 'down' };
...
export function computeStability(current, previous) {
    ...
    for (const entry of current) {
        if (prevIds.has(entry.clusterId))
            carriedOver += 1;
    }
