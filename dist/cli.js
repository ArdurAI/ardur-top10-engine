...
function emitError(code, message, stage, detail, jsonErrors) {
    ...
    process.stderr.write(`ardur-top10-engine: [${code}] ${stage}: ${message}${detail ? ` — ${detail}` : ''}\n`);
...
function readPath(pathOrDash, label, jsonErrors) {
    if (pathOrDash === '-') {
        try {
            return readStdin();
        }
        catch (e) {
            return emitError('PARSE_ERROR', `failed to read stdin for ${label}`, `input:${label}`, e instanceof Error ? e.message : String(e), jsonErrors);
        }
    }
    try {
        ...
        return emitError('PARSE_ERROR', `failed to read file for ${label}: ${pathOrDash}`, `input:${label}`, e instanceof Error ? e.message : String(e), jsonErrors);
    }
}
function parseJson(raw, label, jsonErrors) {
    try {
        return JSON.parse(raw);
    }
    catch (e) {
        return emitError('PARSE_ERROR', `JSON parse failed for ${label}`, `input:${label}`, e instanceof Error ? e.message : String(e), jsonErrors);
    }
}
function loadArtifact(path, stage, label, jsonErrors) {
    const raw = readPath(path, label, jsonErrors);
    const parsed = parseJson(raw, label, jsonErrors);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        return emitError('VALIDATION_ERROR', `expected a JSON object for ${label}`, `input:${label}`, undefined, jsonErrors);
    }
    let result;
    try {
        ...
        return emitError('VALIDATION_ERROR', `schema validation failed for ${label}`, `input:${label}`, e instanceof Error ? e.message : String(e), jsonErrors);
    }
...
function main() {
    ...
    if (!args.ranking) {
        emitError('USAGE_ERROR', '--ranking <file|-> is required', 'cli', 'Usage: cli.ts --ranking <file|-> [--previous <file|->] [--aggregation <file|->] [--now <iso>] [--run-id <id>] [--provider <name>] [--out <file|->] [--json-errors]', args.jsonErrors);
    }
    const ranking = loadArtifact(args.ranking, 'ranking', 'ranking', args.jsonErrors);
    let previous = null;
    if (args.previous && args.previous !== '-') {
        previous = loadArtifact(args.previous, 'top10', 'previous', args.jsonErrors);
    }
    ...
    let aggregation;
    if (args.aggregation) {
        aggregation = loadArtifact(args.aggregation, 'aggregation', 'aggregation', args.jsonErrors);
    }
    const selectionOpts = {};
    if (aggregation)
        selectionOpts.aggregation = aggregation;
    if (args.runId)
        selectionOpts.runId = args.runId;
    if (args.now)
        selectionOpts.generatedAt = args.now;
    let top10;
    try {
        top10 = selectTop10(ranking, previous, selectionOpts);
    }
    catch (e) {
        emitError('SELECTION_ERROR', 'selectTop10 threw unexpectedly', 'selection', e instanceof Error ? e.message : String(e), args.jsonErrors);
    }
